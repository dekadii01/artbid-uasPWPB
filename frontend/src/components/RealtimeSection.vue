...
const ajukanTawaran = () => {
  // Logic untuk mengajukan tawaran baru
...
<template>
  ...
  >
    ...
    ></div>
    <div
      class="absolute -bottom-40 -left-40 w-96 h-96 bg-white/5 rounded-full blur-3xl pointer-events-none"
    ></div>
    ...
    <div class="relative grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
      <div>
        <span
          ...
          >Pengalaman Realtime</span
        >
        <h2 class="text-4xl font-bold mt-3 text-white leading-tight">
          Bersaing Secara<br />Langsung, Tanpa Jeda
        </h2>
        <p class="text-white/50 mt-5 leading-relaxed text-sm">
          Teknologi WebSocket kami memastikan setiap tawaran, pergerakan harga,
          dan setiap detik countdown terjadi secara instan — tanpa perlu
          menyegarkan halaman.
        </p>
      ...
      <div class="bg-white/8 border border-white/10 rounded-2xl p-6">
        ...
        <p class="text-white/40 text-xs mb-3 uppercase tracking-widest">
          Aktivitas Tawaran
        </p>
        <div class="space-y-2">
          <div
          ...
          >
            ...
            <span :class="[bid.amountClass, 'text-xs font-semibold']">{{
              bid.amount
            }}</span>
        ...
        <button
          class="cursor-pointer mt-5 w-full py-3 bg-white text-black rounded-lg text-sm font-medium hover:bg-gray-100 transition-colors"
          @click="ajukanTawaran"
        >
          Ajukan Tawaran
        </button>
